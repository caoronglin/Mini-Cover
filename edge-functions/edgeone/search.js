/**
 * 腾讯云EdgeOne边缘函数 - 搜索API
 *
 * 文档参考:
 * https://cloud.tencent.com/document/product/1552/127366
 */

// EO环境变量
const BING_API_KEY = typeof EO !== 'undefined' && EO.env ? EO.env.BING_API_KEY : '';
...
async function handleSearch(url, ctx) {
  ...
  try {
    ...
    if (typeof caches !== 'undefined') {
      ctx?.waitUntil?.(
        caches.open('eo-search').then(cache => cache.put(cacheKey, response.clone()))
      );
    }
